...
*Для нагруженных проектов можно сбросить параметр **load_configuration** в false. В этом случае
конфигурация роутов будет грузиться не при инициализации, а при первом запросе к менеджеру.*

*Для других драйверов кеширующих механизмов можно указать другие дополнительные общие опции, например
параметры доступа к серверу (серверам) Memcached*
...
1. **data** – хранение одного объекта по id. Если в правиле установить параметр *ipf* отличный от 1,
то в одном хранилище (файле) будет размещаться указанное количество объектов. Также отличается там,
что данные отдаются в виде массива данных одного объекта, а не массива массовов данных объекта.
...
**path** - путь, по которому будет находиться кеш (только для файлового кеша).
...
3. Перевести на работу в качестве сервиса
4. Покрыть тестами
5. Добавить PEAR пакет в канал Symfony для установки через PEAR
6. Выделить плагин в независимый компонент для использования вне фреймворка Symfony, а также в Symfony 2